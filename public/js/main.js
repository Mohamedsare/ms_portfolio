// Loader — Next.js charge ce script après `load` : l’événement ne se reproduit pas,
// il faut lancer la séquence tout de suite si la page est déjà chargée.
...
// Filet de sécurité : éviter un blocage si le loader ne s’est pas fermé
setTimeout(() => {
    const loader = document.querySelector('.loader');
...
if (cursor && cursorFollower) {
    ...
    const hoverElements = document.querySelectorAll(
        'a, button, .skill, .project-card, .filter-btn, .navbar-toggle, .dot, .scroll-top',
    );
...
// Texte animé (machine à écrire) : géré par React — src/components/site/hero-typed-text.tsx
...
// Filtrage des projets (cartes rendues côté serveur / Supabase)
...
// Animation de particules (hero) : gérée par React — voir src/components/site/hero-particles.tsx
